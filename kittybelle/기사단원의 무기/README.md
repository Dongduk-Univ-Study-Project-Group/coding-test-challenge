...
- 약수의 개수를 구하는 효율적인 방법
    - 1 ~ n까지가 아닌, n의 제곱수까지 반복
    - 이유 : 제곱근이 아닌 약수는 짝이 있다. 
        - ex : 1, 2, 5, 10 -> 1과 10, 2와 5
- 문제 조건인 `limit`보다 커질 경우 뒤에는 더 볼 필요도 X, power 바로 return
- for문에서 제곱수를 구하는 과정에서도 Math.sqrt() 사용하면 시간 초과.
    - i * i <= number 일 때로 i를 활용해서 짜야 함.