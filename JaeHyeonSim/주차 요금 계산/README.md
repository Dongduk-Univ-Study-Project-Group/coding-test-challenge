...
1. 입차 시각을 map에 기록한다.
2. 출차 시 입차 시각을 가져와 누적 주차 시간을 계산한다.
    
    이때 입차 기록을 map에서 삭제한다.
    
3. 입차 기록이 남아있는 차들은 23:59 출차로 계산한다.

- 추가 요금 계산 시 소수점은 올림으로 계산해야 한다.
...
| 테스트 1 〉 | 통과 (0.56ms, 75MB) |
| --- | --- |
| 테스트 2 〉 | 통과 (0.43ms, 75.2MB) |
| 테스트 3 〉 | 통과 (0.90ms, 72.3MB) |
| 테스트 4 〉 | 통과 (1.03ms, 77.7MB) |
| 테스트 5 〉 | 통과 (1.81ms, 72.4MB) |
| 테스트 6 〉 | 통과 (1.98ms, 72.7MB) |
| 테스트 7 〉 | 통과 (14.58ms, 89.3MB) |
| 테스트 8 〉 | 통과 (6.58ms, 73.8MB) |
| 테스트 9 〉 | 통과 (2.45ms, 72.9MB) |
| 테스트 10 〉 | 통과 (7.06ms, 76.7MB) |
| 테스트 11 〉 | 통과 (6.56ms, 80.2MB) |
| 테스트 12 〉 | 통과 (7.09ms, 81.2MB) |
| 테스트 13 〉 | 통과 (0.54ms, 75.6MB) |
| 테스트 14 〉 | 통과 (0.76ms, 84.8MB) |
| 테스트 15 〉 | 통과 (0.55ms, 77MB) |
| 테스트 16 〉 | 통과 (0.26ms, 78.6MB) |
